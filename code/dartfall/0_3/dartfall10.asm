...
cp c
jp c,  mayor	;;Cuando C es mayor que A se va por 'c' carry
and a		;;Resetear el flag carry
cp c
jp z,  igual	;;Cuando C es igual a A se va por igual
and a		;;Resetear el flag carry
cp c
jp nc, menor 	;;Cuando C es menor o igual a A se va por 'nc' nocarry
call continuar
ret
mayor:
ld hl, #0301
CALL #BB75               ;;Llama al set cursor
ld a,#31
call printchar
call #BB7E               ;;Deshabilita cursor para usuario
call #bb84               ;;Deshabilita cursor para sistema
call continuar
ret
igual:
ld hl, #0301
...
;;
;;-------------------------------------------------;;
;; Guarda el valor de la pos mem. video en hl      ;;
;;-------------------------------------------------;;
.poshl: db #00,#00
.cont1: db #00
;; A5 diana izquierda
...
db #30,#30,#30,#30
;;   N   i   v   e   l       -       0   1
db #4E,#69,#76,#65,#6C,#80,#2D,#80,#30,#31
ret
;;
.pinta4pix8lins:
ld (hl), b
...
;;						
;;-------------------------------------------------;;						
;; Pinta barra amarilla abajo de la barra superior ;;						
;;-------------------------------------------------;;						
;;
ld hl, #c850
ld a , #50		
...
;;
;;-------------------------------------------------;;						
;;    BLANCO PARA EL NIVEL 1                       ;;						
;;-------------------------------------------------;;						
;;
ld hl, #f7A6             ;; Diana principal 
ld (hl), #FF
;inc hl
;ld (hl), #FF
;;
ld hl, #ffA6             ;; Diana principal
ld (hl), #FF
;inc hl
;ld (hl), #FF
;;
ld hl, #f7A5             ;; Diana secundaria izquierda
ld (hl), #0F
;inc hl
;ld (hl), #0F
;;
ld hl, #fFA5             ;; Diana secundaria izquierda
ld (hl), #0F
;;inc hl
;;ld (hl), #0F
;;
ld hl, #f7A7             ;; Diana secundaria derecha
ld (hl), #0F
;inc hl
;ld (hl), #0F
;;
ld hl, #fFA7             ;; Diana secundaria derecha
ld (hl), #0F
;inc hl
;ld (hl), #0F
;
...
;;
;;-------------------------------------------------;;						
;;    MOVIMIENTO DARDO PARA EL NIVEL 1             ;;						
;;-------------------------------------------------;;						
;;
.nivel1:
;;ld hl,#d052			;; A2 El dardo se ubica en la posicion inicial
   ld a,#4D   ;;#4c
   ld (cont1),a                 ;; Guarda en cont1 el valor de a
   ld hl,#C0A1
   call loop1
   ld a,#4D   ;;#4c
   ld (cont1),a                 ;; Guarda en cont1 el valor de a
